...
def create_uniform_grid(low, high, bins=(10, 10)):
    ...
    """
    return np.array([np.arange(low[0] + (high[0] - low[0]) / bins[0], high[0], (high[0] - low[0]) / bins[0]),                      np.arange(low[1] + (high[1] - low[1]) / bins[1], high[1], (high[1] - low[1]) / bins[1])])
...
class QLearningAgent:
    ...
    def preprocess_state(self, state):
        ...
        print(state)
        print(state_grid)
        print(discretize(state, self.state_grid))
    ...
    def act(self, state, reward=None, done=None, mode='train'):
        ...
        if mode == 'test':
            ...
            print('max', max(self.q_table[state]))
