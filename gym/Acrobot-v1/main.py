...
samples = [(-1.2 , -5.1 ),
           ...
           ( 0.7 , -3.7 ),
           ( 1.0 ,  5.0 )]
encoded_samples = [tile_encode(sample, tilings, False) for sample in samples]
...
class TiledQTable:
    ...
    def get(self, state, action):
        ...
        value = 0.0
        for q_table, index in zip(self.q_tables, encoded_state):
            value += q_table.q_table[tuple(index + (action,))]
    ...
    def update(self, state, action, value, alpha=0.1):
        ...
        for q_table, index in zip(self.q_tables, encoded_state):
            q_table.q_table[tuple(index + (action,))] = alpha * value                                 + (1.0 - alpha) * q_table.q_table[tuple(index + (action,))]
